...
function SetTerrainFromImage($filename, $terrainIds, $forestDensity = 100){
    ...
    for($y = 0; $y < $mapSize; $y++)
        for($x = 0; $x < $mapSize; $x++){
            ...
            if($id !== false && isset($terrainIds[$id][0])){
                # Set terrain
                SCX::$data_serial['terrain']['data'][$x.','.$y][1] = $terrainIds[$id][0];
                # Add tree for forest terrain
                if(key_exists($terrainIds[$id][0], LIB::$TerrainTrees) && ($forestDensity == 100 || rand(0, 100) < $forestDensity)){
                    $tree = LIB::$TerrainTrees[$terrainIds[$id][0]];
                    NewObject(0,$tree[0], array($x + 0.5,$y + 0.5), 0, -1, rand(0, $tree[1]));
                }
                /* -> ZT start */
                if($terrainIds[$id][1]!=0)
                {
                    /* create new objects */
                    NewObject(0,$terrainIds[$id][1], array($x + 0.5,$y + 0.5), 0, -1, 0);                                            
                }
                /*<- ZT end */
            }
        }
